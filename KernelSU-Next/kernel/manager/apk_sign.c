                                               ...
                                               const char *expected_sha256)
{
	...
	/* O_NOATIME so scanning other apps' APKs does not touch their atime. Both
	 * callers run under override_creds(ksu_cred), which clears the owner check. */
	struct file *fp = ksu_filp_open_compat(path, O_RDONLY | O_NOATIME, 0);
	if (IS_ERR(fp)) {
		pr_err("open %s error.\n", path);
		return false;
	}
	...
	// single buffered read; the upstream byte-by-byte /data/app scan stalls boot
	{
	...
	// walk the length-prefixed pairs, like AOSP's signing-block parser
	while (pos < pairs_end) {
		uint32_t id;
		u64 size_of_pair;
		loff_t pair_end;
